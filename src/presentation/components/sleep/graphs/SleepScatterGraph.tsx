...
import { Brush, CartesianGrid, Scatter, ScatterChart, Tooltip, XAxis, YAxis, ZAxis } from 'recharts';
...
class SleepScatterGraph extends Component<SleepScatterGraphProps, SleepScatterGraphState> {
  ...
  private getBrushFilteredData() {
    ...
    return this.props.data.filter(datum =>
      new Date(datum.date) >= new Date(brushStartIndex) && new Date(datum.date) <= new Date(brushEndIndex)
    );
  }
