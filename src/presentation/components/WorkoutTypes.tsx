...
import { PieChart, ResponsiveContainer, Sector } from 'recharts'
...
class WorkoutTypes extends Component<WorkoutTypeProps, WorkoutTypesState> {
  ...
  extractWorkoutTypes() {
    ...
    const uniqueTypes = types.filter((v, i, self) => {
      return self.indexOf(v) === i
    })
    const result = uniqueTypes.filter(it => {
      return it
    }).map(type => {
      if (counts[type] > 5) {
        return {
